...
class BookingForm(forms.ModelForm):
    ...
    booking_time = forms.TimeField(widget=forms.TimeInput(format='%H:%M', attrs={'type': 'time'}), label="Booking Time")
    booking_date = forms.DateField(widget=forms.TextInput(attrs={'type': 'date'}), label="Booking Date")
    number_of_guests = forms.IntegerField(widget=forms.NumberInput(attrs={'min': '1'}), label="Number of Guests")
    ...
    def clean_number_of_guests(self):
        ...
        if number_of_guests < 1:
            raise forms.ValidationError("Number of guests cannot be negative or zero.")
